...
def _coerce(x):
    """Unwrap a 0-d numpy array back to its native Python scalar/object.

    aDDM continuation controls (``continuation_mode``/``continuation_params`` and
    ``fixation_dist``/``fixation_params``) may arrive wrapped in a 0-d array when HSSM threads
    them through the ``extra_fields`` transport (which per-trial-broadcasts its entries). This
    restores the ``str``/``dict``/``None`` the strategy resolvers expect. (Self-sampled fixation
    schedules are generated by ``ssms.basic_simulators.fixation_continuation.generate_schedule``,
    the distribution-generic successor to efpt's gamma-only ``_generate_sacc_array_data``.)
    """
...
def addm(eta, kappa, a, b, x0, t, deadline, s,
         ...
         fixation_dist='gamma',
         fixation_params=None,
         continuation_mode='prolong_last_fixation',
    ...
    Fixation-duration distributions are pluggable (positive ``scipy.stats`` dists,
    see ``ssms.basic_simulators.fixation_continuation``):
    * Mode 1 self-samples its schedule from ``fixation_dist``/``fixation_params``
      (default gamma via ``gamma_shape``/``gamma_scale``).
    * Mode 2 (PPC) tail behaviour is ``continuation_mode``: ``prolong_last_fixation``
      (default; hold the last gaze), ``sample_continuation`` (draw the tail from
      ``continuation_params['dist']``), or ``resample_all_fixations`` (self-sample the
      whole fixation behaviour, keep r1/r2). ``continuation_params`` is a
      ``{'dist': <name>, 'dist_params': {...}}`` dict (None for ``prolong_last_fixation``).
    ...
    # Isolated stream for the continuation strategies: the primary rng (ratings + trajectory
    # seeds) is left untouched, so `prolong_last_fixation` reproduces the pre-feature output
    # byte-for-byte. Lazy import avoids any package load-order cycle with ssms.basic_simulators.
    from ssms.basic_simulators.fixation_continuation import (
        resolve_continuation_mode, generate_schedule,
    )
    cont_rng = np.random.default_rng(np.random.SeedSequence(seed).spawn(1)[0])
    ...
    if mode2:
        ...
        # Pluggable fixation continuation (PPC): prolong_last_fixation (default; no-op),
        # sample_continuation, or resample_all_fixations. Draws use the isolated cont_rng, so
        # the default is byte-identical to the pre-feature path. resample_all_fixations replaces
        # node_r/d_r/flag_r (self-sampled) while r1_r/r2_r (stimulus) stay observed.
        node_r, d_r, flag_r, max_d = resolve_continuation_mode(_coerce(continuation_mode))(
            node_r, d_r, flag_r, T, _coerce(continuation_params), cont_rng)
    else:
        # Mode 1: self-sample ratings + first gaze per row; self-sample the fixation schedule
        # from the chosen positive distribution (default gamma via gamma_shape/gamma_scale).
        ...
        fp = fixation_params if fixation_params is not None else {"a": gamma_shape, "scale": gamma_scale}
        node_r, d_r, max_d = generate_schedule(N, T, _coerce(fixation_dist), _coerce(fp), rng)
